...
> [!WARNING]
> 程序没有针对“污染”和“异色”精灵做专门识别或排除处理。识别仅基于图片相似度；如果外观差异不大且精灵没有上锁，它们也可能被一起选中。批量选择完成后，请务必在游戏内仔细检查，确认没有误选后再执行放生。
...
6. 自动选择结束后，在游戏内逐一检查已选精灵，特别注意污染、异色和其他不希望放生的精灵，确认无误后再放生。
7. 随时按 `Esc`、点击界面的“停止”，或把鼠标移到整个桌面的左上角，即可紧急停止。
...
下面是当前噼啪鸟模板使用的截图。建议参照这个大小和范围截取其他精灵：

| 未选中模板 | 已选中模板 |
| --- | --- |
| ![噼啪鸟未选中模板](assets/unselected.png) | ![噼啪鸟已选中模板](assets/selected.png) |
...
> 自动点击会真实控制鼠标。程序仅负责选中精灵以方便后续放生，不会修改任何游戏数据，也不会专门排除污染或异色精灵。调试模板和阈值时请优先使用“仅检测（不点击）”，放生前请再次人工核对。
